...
@include('layouts.partials.aside')
                    ...
                    <div class="card my-4 shadow-lg border-0">
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                            <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3">
                                <h6 class="text-white text-capitalize ps-3">Asignar Suscripción</h6>
                            </div>
                        </div>
                    ...
                    <div class="container d-flex justify-content-center">
                        <div class="row justify-content-center">
                            <div class="col-md-12">
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <h5 class="card-title text-center mb-4">Formulario de Suscripciones</h5>
                                        <form id="formulario" onsubmit="mostrarConfirmacion(event)">
                                            <div class="form-row">
                                                <div class="form-group col-md-4 me-4">
                                                    <label for="cliente" class="text-black fw-bold me-6">
                                                        Profesional</label>
                                                ...
                                                <div class="form-group col-md-4">
                                                    <label for="sus" class="text-black fw-bold">
                                                        Suscripción</label>
                                                    <select id="sus" class="lista" onchange="calcularFechas()">
                                                        <option selected>Seleccionar</option>
                                                        <option value="gratis">Gratis - $0</option>
                                                        <option value="mensual">Mensual - $100</option>
                                                        <option value="anual">Anual - $900</option>
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-row">
                                                <div class="form-group col-md-4 me-4">
                                                    <label for="fechaInicio" class="text-black fw-bold">Fecha de
                                                        Inicio</label>
                                                    <input type="datetime-local" class="form-control custom-input"
                                                        id="fechaInicio" placeholder="Fecha de Inicio"
                                                        onchange="calcularFechas()">
                                                </div>

                                                <div class="form-group col-md-4">
                                                    <label for="fechaTermino" class="text-black fw-bold">Fecha de
                                                        Termino</label>
                                                    <input type="datetime-local" class="form-control custom-input"
                                                        id="fechaTermino" placeholder="Fecha de Termino" readonly>
                                                </div>
                                            </div>
                        ...
                        <div id="confirmacionModal" class="modal-overlay" style="display: none;">
                            <div class="card modal-card">
                                <div class="card-body">
                                    ...
                                    <p class="card-text text-center">La suscripción ha sido agregada con éxito.</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        function calcularFechas() {
            // Obtener la fecha de inicio y la suscripción seleccionada
            ...
            if (fechaInicioInput.value) {
                ...
                // Calcular la fecha de término según la suscripción
                let fechaTermino;
                switch (suscripcion) {
                    case 'mensual':
                        fechaTermino = new Date(fechaInicio);
                        fechaTermino.setMonth(fechaInicio.getMonth() + 1); // Añadir un mes
                        break;
                    case 'anual':
                        fechaTermino = new Date(fechaInicio);
                        fechaTermino.setFullYear(fechaInicio.getFullYear() + 1); // Añadir un año
                        break;
                    case 'gratis':
                        fechaTermino = fechaInicio; // No aplica un término específico para gratis
                        break;
                    default:
                        fechaTermino = fechaInicio; // Si no se ha seleccionado una suscripción, la fecha de término es igual a la de inicio
                        break;
                }

                // Actualizar la fecha de término en el input
                const year = fechaTermino.getFullYear();
                const month = (fechaTermino.getMonth() + 1).toString().padStart(2, '0');
                const day = fechaTermino.getDate().toString().padStart(2, '0');
                const hour = fechaTermino.getHours().toString().padStart(2, '0');
                const minute = fechaTermino.getMinutes().toString().padStart(2, '0');

                fechaTerminoInput.value = `${year}-${month}-${day}T${hour}:${minute}`;
            }
        }
    </script>
</body>
